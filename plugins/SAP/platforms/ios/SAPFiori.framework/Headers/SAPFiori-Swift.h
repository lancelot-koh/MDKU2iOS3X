...
///    }
///
///    // MARK: Implement alternative input
///
///    cell.isAlternativeInputEnabled = true
///    cell.alternativeInputButton.setImage(myImage, for: .normal)
///    cell.alternativeInputButton.didSelectHandler = { [weak self] (button) in
///        print("Tapped")
///    }
///
///    return cell
...
/// The <code>FUITextRecognitionView</code> can be used to perform text recognition using device camera. Video captured from device camera would be shown in in entire bounds. Use <code>captureMaskPath</code> to define the sub-frame of the view, in which the text would be recognized. Rest of the frame would be dimmed by <code>overlayView</code>. This default behaviour can be changed by setting view and layer properties of <code>overlayView</code>. All the observations i.e. recogznied text blocks and corresponding bounding boxes, would be returned back to <code>completionHandler</code>. To provide hint to user to what texts are recognized, use <code>showTexts</code> to show the observations on top of captured video.
/// Call <code>startSession</code> to start capturing the video. This is normally called when view appears. Since text recognition model are memory and cpu intensive, it wont be run untill user has stablized the device camera. After that <code>completionHandler</code> would be called for every video frame with observations. <code>completionHandler</code> can be used to perform any validations and filtering on the observations. Validate if it is a credit card number if observations has 16 numberic digits in total and discard any other observations, for example. Once the filtered observations meet the desired criteria, call <code>stopSession</code> to stop capturing the video. Make sure to call <code>stopSession</code> in case view disappears.
/// <h2>Example Initialization and Configuration</h2>
/// \code
///
/// let recognitionView = FUITextRecognitionView()
/// recognitionView.captureMaskPath = {
///    let path = UIBezierPath(roundedRect: CGRect(x: 0, y: 0, width: 311, height: 44), cornerRadius: 4)
///    path.lineWidth = 1
///    return path
/// }()
/// recognitionView.completionHandler = { observations
///
...
///   </li>
/// </ul>
SWIFT_CLASS("_TtC8SAPFiori22FUITextRecognitionView")
...
///
/// var frameCount = 100 //capture the observations after 100 frames
///
...
///    let matches = detector.matches(in: observations.map { $0.0 })
///
///    textRecController.recognitionView.showTexts(for: observations.map { $0.0 }, with: matches)
///
///        frameCount -= 1
///        if frameCount < 0 {
///            DispatchQueue.main.async {
...
///
/// \endcode
SWIFT_CLASS("_TtC8SAPFiori32FUITextRecognitionViewController")
...
/// the <code>title</code> property,<code>body</code> containing the details, an <code>actionTitle</code> ,which when clicked invokes the
...
SWIFT_CLASS("_TtC8SAPFiori24RecognizeTextObservation")
@interface RecognizeTextObservation : VNRectangleObservation
/// :nodoc:
